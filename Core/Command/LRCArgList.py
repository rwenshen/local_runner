...
class LRCArgList(LRCore.LRLogger):
    ...
    def __getattr__(self, name):
        ...
        self.logCritical(f'Argument "{name}" is NOT existent! "None" will be returned.')
        return None
    ...
    def __setArg(self, name, value):
        ...
        if name not in self.__myDict:
            self.logCritical(f'Argument "{name}" is NOT existent! The setting will be ignored.')
            return
        # get arg
        arg = LRCArg.sGetArg(name)
        # check type
        if value is not None and not isinstance(value, arg.myType):
            self.logCritical(f'Value "{value}" is NOT in type of {arg.myType} for argument "{name}"! The setting will be ignored.')
        # check choice
